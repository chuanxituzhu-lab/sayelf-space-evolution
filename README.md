...
一个本地优先的「手绘稿 → 草图整理 → 线落成墙 → 空间成型 → 视频分镜」工作台。
...
1. 上传手绘稿。
2. 输入一句空间描述。
3. 保持默认 5 项 Spatial Continuity Lock。
4. 点击「生成完整工作流」。
5. 按 01 → 02 → 03 → 04 顺序，把提示词和上一阶段图片交给你正在使用的 AI 图像/视频工具。
6. 视频阶段优先用阶段 03 成图作为首帧。

## v1.0 边界
本版本是可本地运行的商业工作流版，负责：输入、空间一致性规则、提示词/分镜编排、项目保存与导出。
...
本产品是：
`Sketch → Line Cleanup → Line Becomes Wall → Space → Film`
